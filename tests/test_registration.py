...
def _catalog_line(description: str, max_chars: int = 60) -> str:
    ...
    Mirrors tools/tool_search.py::_short_desc. Plugin tools are deferred
    behind tool_search, so this one line is everything the model sees until it
    calls tool_describe. If the credit cost does not fit here, the steering is
    invisible at exactly the moment the tool gets chosen.
    """
    import re
...
class TestCatalogLine:
    ...
    def test_the_free_tools_say_so_in_the_catalog(self):
        for name in self.FREE:
            line = _catalog_line(schemas.ALL_SCHEMAS[name]["description"])
            assert "free" in line.lower(), f"{name}: {line!r}"

    def test_the_costly_tools_say_so_in_the_catalog(self):
        for name in ("shodan_search", "shodan_scan"):
            line = _catalog_line(schemas.ALL_SCHEMAS[name]["description"])
            assert "credit" in line.lower(), f"{name}: {line!r}"

    def test_count_and_search_point_at_each_other(self):
        """The two overlap, and one is free while the other is not."""
        assert "free" in _catalog_line(schemas.SHODAN_COUNT["description"]).lower()
        assert "count" in _catalog_line(schemas.SHODAN_SEARCH["description"]).lower()

    def test_all_three_skills_are_discoverable_from_the_tools(self):
        ...
        for skill in ("shodan:query-syntax", "shodan:recon", "shodan:monitoring"):
            assert skill in blob, skill
